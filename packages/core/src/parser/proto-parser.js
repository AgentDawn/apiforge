/**
 * Lightweight Protocol Buffers (.proto) file parser.
 * Extracts services, methods, messages, and enums from proto3 files.
...
export class ProtoParser {
  ...
  /**
   * Parse a .proto file from text.
   * @param {string} input - Raw .proto file content
   * @returns {ProtoParser}
   */
  static parse(input) {
  ...
  /** @returns {string} raw proto file content */
  ...
  /** @returns {object} message name → MessageDef */
  getMessages() { return this.#result.messages; }

  /** @returns {object} enum name → EnumDef */
  getEnums() { return this.#result.enums; }

  /**
   * Convert parsed proto to OpenAPI-style endpoint array
   * compatible with APIForge's loadSpec/renderCollectionTree.
  ...
  /**
   * Generate a spec-like object for compatibility with loadSpec().
   * @returns {object}
   */
  toSpec() {
  ...
  #doParse(input) {
    ...
    // Strip comments but preserve line structure
  ...
  #tokenize(input) {
    // Simple tokenizer: split on whitespace and punctuation, preserve strings
    const tokens = [];
    let i = 0;
    while (i < input.length) {
      // Skip whitespace
  ...
  #parseTokens(tokens, result) {
    ...
    const expect = (value) => {
      const t = next();
      if (!t || t.value !== value) throw new Error(`Expected '${value}' but got '${t?.value}'`);
      return t;
    };
    const skipUntil = (value) => {
      while (i < tokens.length && tokens[i].value !== value) i++;
      if (i < tokens.length) i++; // skip the target
    };
  ...
  #parseField(tokens, startIdx) {
    ...
    return {
      def: { name, type, repeated, optional, map: mapType },
      nextIndex: i,
    };
  }
    ...
    for (const field of msgDef.fields) {
      if (field.repeated) {
        properties[field.name] = {
          type: 'array',
          items: { type: this.#protoTypeToJsonType(field.type) },
        };
      } else if (field.map) {
        properties[field.name] = {
          type: 'object',
          additionalProperties: { type: this.#protoTypeToJsonType(field.map.valueType) },
        };
      } else {
  ...
  #protoTypeToJsonType(protoType) {
    const map = {
      ...
      'float': 'number',
      'int32': 'integer',
      'int64': 'string', // int64 is string in JSON representation
      'uint32': 'integer',
      'uint64': 'string',
      'sint32': 'integer',
      'sint64': 'string',
    ...
    return map[protoType] || 'object'; // unknown types are message references
  }
}
